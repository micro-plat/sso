...
//Query 获取用系统列表
func (u *DbSystem) Query(page int,name string,status string) (data db.QueryRows, count interface{}, err error) {
	db := u.c.GetRegularDB()
	count, q, a, err := db.Scalar(sql.QuerySubSystemTotalCount,  map[string]interface{}{
		"name": name,
		"enable": status,
	})
	if err != nil {
