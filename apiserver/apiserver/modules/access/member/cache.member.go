...
import (
	...
	cachekey "github.com/micro-plat/sso/apiserver/apiserver/modules/const/cache"
)
...
func (l *CacheMember) GetUserInfoByCode(code string) (info string, err error) {
	...
	cachekey := transform.Translate(cachekey.CacheLoginUser, "code", code)
...
func (l *CacheMember) SaveLoginVerifyCode(userName, verfiyCode string, expire int) error {
	cache := l.c.GetRegularCache()
	key := transform.Translate(cachekey.CacheLoginVerifyCode, "user_name", userName)
