...
where 
	t.user_id = @user_id
	and t.belong_type = @belong_type
	and t.belong_id = @belong_id
`
...
insert 
	into sso_user_info 
	(full_name,user_name, status, password, mobile, email, ext_params, belong_id, belong_type)
values
	(@full_name, @mobile, @status, @password, @mobile, @email, @ext_params, @belong_id, @belong_type)
`

//AddAdmin 增加管理员(有些要特殊处理)
const AddAdmin = `
insert 
	into sso_user_info 
	(full_name,user_name, status, password, mobile, email, ext_params, belong_id, belong_type)
values
	(@mobile, @mobile, 0, @password, @mobile, '', '', @belong_id, @belong_type)
`

//AddUserAdminRole 关联管理员角色
const AddUserAdminRole = `
