...
type IDbRole interface {
	...
	QueryRoleInfoByName(roleName string, belongID, belongType int) (data db.QueryRow, err error)
}
...
//由于现在还没有控制到加油站级别，所以belong_type, belong_id可以先不用管，加油站要自己控制角色时就要管了
func (r *DbRole) QueryRoleInfoByName(roleName string, belongID, belongType int) (data db.QueryRow, err error) {
	db := r.c.GetRegularDB(config.DbName)
	result, _, _, err := db.Query(sqls.QueryRoleInfoByName, map[string]interface{}{
		"role_name":   roleName,
