...
<script>
  export default {
    ...
    methods:{
        getUserInfo() {
          this.$http.post("/loginweb/member/bind/info",{user_id:this.userId})
          .then(res => {
            if (res.wx_openid) {
              this.disabled = true;
        ...
        checkUserInfo() {
            ...
            .then(res =>{
                var url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + res.appid + "&state=" + res.state + "&redirect_uri=" +
                        encodeURIComponent(this.$env.conf.system.wxcallbackhost + "/wxcallback") +
...
.top-title{ text-align:center; font-size:18px; color:#333; font-weight:bold; padding:10% 10px 10px 10px;}
.user-info{ margin:10px 10px; padding:10px; border-top:1px solid #efefef;border-bottom:1px solid #efefef; background:#fff;}
